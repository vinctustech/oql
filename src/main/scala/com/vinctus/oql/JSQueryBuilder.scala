...
//    )
  ...
  def getOne(): js.Promise[js.UndefOr[Any]] = check.oql.jsQueryOne(q, null, fixed) //empty parentheses aren't redundant: Scala.js compiler needs them

  def getCount(): js.Promise[Int] = check.oql.count(q, null, fixed).toJSPromise //empty parentheses aren't redundant: Scala.js compiler needs them
