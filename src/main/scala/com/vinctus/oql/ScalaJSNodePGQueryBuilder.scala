...
class ScalaJSNodePGQueryBuilder private[oql] (
    private val oql: OQL_NodePG_ScalaJS,
    private[oql] val q: OQLQuery,
    fixed: Fixed
) {
  private def check = if (q.source eq null) sys.error("QueryBuilder: no source was given") else this

  private class DoNothingQueryBuilder extends ScalaJSNodePGQueryBuilder(oql, q, fixed) {
    private def na = sys.error("not applicable")

    override def cond(b: Boolean): ScalaJSNodePGQueryBuilder = na
...
//    override def ccGetOne[T <: Product: Mappable]: Future[Option[T]] = na
    ...
    override def limit(a: Int): ScalaJSNodePGQueryBuilder = ScalaJSNodePGQueryBuilder.this

    override def offset(a: Int): ScalaJSNodePGQueryBuilder = ScalaJSNodePGQueryBuilder.this

    override def order(attribute: String, sorting: String): ScalaJSNodePGQueryBuilder = ScalaJSNodePGQueryBuilder.this
...
//    override def add(attribute: QueryBuilder): QueryBuilder = SJSQueryBuilder.this
    ...
    override def query(query: String): ScalaJSNodePGQueryBuilder = ScalaJSNodePGQueryBuilder.this

    override def select(s: String): ScalaJSNodePGQueryBuilder = ScalaJSNodePGQueryBuilder.this
  }

  def cond(b: Boolean): ScalaJSNodePGQueryBuilder = if (b) this else new DoNothingQueryBuilder
...
//    )
  ...
  def query(query: String): ScalaJSNodePGQueryBuilder = new ScalaJSNodePGQueryBuilder(oql, oql.parseQuery(query), fixed)

  def select(s: String): ScalaJSNodePGQueryBuilder = {
  ...
  def order(attribute: String, sorting: String): ScalaJSNodePGQueryBuilder = {
    val attr = AttributeOQLExpression(List(Ident(attribute)), null)

    AbstractOQL.decorate(q.entity, attr, oql.model, oql.ds, null)
    new ScalaJSNodePGQueryBuilder(oql, q.copy(order = Some(List(OQLOrdering(attr, sorting)))), fixed)
  }

  def limit(a: Int): ScalaJSNodePGQueryBuilder = new ScalaJSNodePGQueryBuilder(oql, q.copy(limit = Some(a)), fixed)

  def offset(a: Int): ScalaJSNodePGQueryBuilder = new ScalaJSNodePGQueryBuilder(oql, q.copy(offset = Some(a)), fixed)
