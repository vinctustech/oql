...
class ScalaPlainQueryBuilder private[oql] (private val oql: AbstractOQL, private[oql] val q: OQLQuery, fixed: Fixed)(
    implicit ec: scala.concurrent.ExecutionContext) {
  private def check = if (q.source eq null) sys.error("QueryBuilder: no source was given") else this

  private class DoNothingQueryBuilder extends ScalaPlainQueryBuilder(oql, q, fixed) {
...
//    )
  ...
  def query(query: String): ScalaPlainQueryBuilder = new ScalaPlainQueryBuilder(oql, oql.parseQuery(query), fixed)
  ...
  def order(attribute: String, sorting: String): ScalaPlainQueryBuilder = {
    ...
    new ScalaPlainQueryBuilder(oql, q.copy(order = Some(List(OQLOrdering(attr, sorting)))), fixed)
  }

  def limit(a: Int): ScalaPlainQueryBuilder = new ScalaPlainQueryBuilder(oql, q.copy(limit = Some(a)), fixed)

  def offset(a: Int): ScalaPlainQueryBuilder = new ScalaPlainQueryBuilder(oql, q.copy(offset = Some(a)), fixed)
