...
//    conn
  ...
  lazy val db: OQL_NodePG_ScalaJS =
...
//    conn
  ...
  def test(oql: String, parameters: (String, Any)*): Future[String] =
//    if oql startsWith "races" then db.showQuery()

    db.json(oql, parameters = parameters.toMap)
