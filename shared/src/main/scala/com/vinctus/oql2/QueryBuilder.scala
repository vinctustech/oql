...
class QueryBuilder private[oql2] (private val oql: OQL, private[oql2] val q: OQLQuery) {
  ...
  private class DoNothingQueryBuilder extends QueryBuilder(oql, q) {
    ...
    override def getMany(jsdate: Boolean = false): List[Any] = na

    override def getOne(jsdate: Boolean = false): Option[Any] = na

    override def getCount(jsdate: Boolean = false): Int = na
